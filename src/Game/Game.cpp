...
#include "../../include/MediaFolderLoader/CommentaryMediaFolderLoader.h"
...
std::string Game::getOutcomeOnBall(FileReader& obj, int teamBattingId, int teamBowlingId, int batterId, int bowlerId, std::string& overSummary){
    ...
    std::map<std::string, std::map<std::string, std::string>> shotTimingMap;
    std::map<std::string, std::string> timingOutcomeMap;
    std::string outcomesString;
    try{
        shotTimingMap = outcomesOnBowlCard.at(inp.getBowlCard());
    }catch(std::out_of_range& e){
        std::cerr<<e.what()<<"\n";
        std::cout<<"error while reading the input file...bowlcard mentioned in the input file is not correct";
        exit(0);
    }
    try{
        timingOutcomeMap = shotTimingMap.at(inp.getShot());
    }catch(std::out_of_range& e){
        std::cerr<<e.what()<<"\n";
        std::cout<<"error while reading the input file...shot mentioned in the input file is not correct";
        exit(0);
    }
    try{
        outcomesString = timingOutcomeMap.at(inp.getTiming());
    }catch(std::out_of_range& e){
        std::cerr<<e.what()<<"\n";
        std::cout<<"error while reading the input file...timing mentioned in the input file is not correct";
        exit(0);
    }
    std::vector<std::string> outcomes = stringSplit(outcomesString, CODE_VARIABLES::OUTCOMES_SEPARATOR_IN_MEDIA_FILE);
    ...
    try{
        comments = commentary.at(outcome);
    }catch(std::out_of_range& e){
        std::cerr<<e.what()<<"\n";
        std::cout<<"commentary is not mentioned for the given outcome in the media files\n\n";
        exit(0);
    }
    srand(time(NULL));
...
void Game::startSuperOver(){
    ...
    // first innings
    commentaryLine = "Let's begin " + teams[teamBattingFirstId].teamName + " innings\n\n"; 
    std::cout<<commentaryLine;
    ...
    // second innings
    commentaryLine = "Let's begin " + teams[teamBowlingFirstId].teamName + " innings\n\n";
    std::cout<<commentaryLine;
    std::cout<<"Hit enter to continue\n\n";
    std::cin.ignore();
    instance->commentaryWriter->appendNextLine(commentaryLine);
